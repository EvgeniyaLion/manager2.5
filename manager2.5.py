...
def timer():
    try:
        ...
        finish = datetime.now()
        implementation = finish - start
        print(f"Время окончания работы кода: {finish}")
        print(f"Время, затраченное на выполнение кода: {implementation}")
...
with timer() as start:
    print(f"Время запуска кода: {start}")
